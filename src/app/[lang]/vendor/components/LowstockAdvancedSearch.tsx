...
export function LowstockAdvancedSearchOverlay({ onChange, confirm }: Props) {
  ...
  return (
    <OverlayLayout className="max-h-screen overflow-auto" close={() => setIsAdvancedSearchOpen(false)}>
      ...
      <LowstockAdvancedSearch confirm={confirm} onChange={onChange} />
    </OverlayLayout>
  );
}

export function LowstockAdvancedSearch({ onChange, confirm }: Props) {
  const { t } = useTranslation();
  return (
    <>
      <Dropdown
